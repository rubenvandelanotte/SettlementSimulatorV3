...
class RuntimeAnalyzer:
    ...
    def run(self):
        if not self.suite.runtime_data:
        ...
        self._plot_average_runtime_with_stddev(runtime_df)
        self._plot_runtime_boxplot(runtime_df)
        self._plot_avg_runtime_line_with_stddev(runtime_df)
        self._plot_runtime_efficiency_multiline(runtime_df)
        self._plot_correlation_heatmap(runtime_df)
        self._plot_global_runtime_histogram(runtime_df)
    ...
    def _build_runtime_dataframe(self):
        ...
        for entry in self.suite.runtime_data:
            ...
            if run_label and "_" in run_label:
                config_key = run_label.split("_")[0]
            instruction_eff = None
            value_eff = None

            # Match efficiency from statistics if available
    ...
    def _plot_average_runtime_with_stddev(self, df):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, "runtime_bars.png"), dpi=300)
        plt.close()
    ...
    def _plot_runtime_boxplot(self, df):
        ...
        plt.grid(axis='y', linestyle='--', alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, "runtime_boxplot.png"), dpi=300)
        plt.close()

    def _plot_avg_runtime_line_with_stddev(self, df):
        avg_runtime = df.groupby("config")["execution_time"].mean()
        std_runtime = df.groupby("config")["execution_time"].std()
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, "runtime_by_config.png"), dpi=300)
        plt.close()
    ...
    def _plot_runtime_efficiency_multiline(self, df):
        ...
        plt.xticks(rotation=45, ha='right')
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, "runtime_and_efficiency.png"), dpi=300)
        plt.close()
    ...
    def _plot_correlation_heatmap(self, df):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, "runtime_correlation_heatmap.png"), dpi=300)
        plt.close()

    def _plot_global_runtime_histogram(self, df):
        plt.figure(figsize=(12, 8))
        plt.hist(df["execution_time"], bins=30, color='skyblue', edgecolor='black')
        plt.xlabel('Runtime (seconds)')
        plt.ylabel('Frequency')
        plt.title('Global Runtime Distribution')
        plt.grid(axis='y', linestyle='--', alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, "runtime_histogram.png"), dpi=300)
        plt.close()
